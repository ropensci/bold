...
b_url <- function(api) {
  paste0("https://v4.boldsystems.org/index.php/", api)
}
b_url_async <- function(api, query){
  n <- curl::curl_escape(names(query))
  param <- query[[1L]]
  params <- query[-1L]
  query <- vapply(param, function(x) {
    query <- curl::curl_escape(x)
    paste0(n, "=", c(query, params), collapse = "&")
  }, character(1L))
  `names<-`(
  ...
  function(...,
    ...
    if (!length(params))
      stop(
        ...
        b_ennum(paramnames, join_word = "or", quote = TRUE),
        call. = FALSE
      )
    wt <- !vapply(params, is.character, logical(1))
    if (any(wt))
      stop(b_ennum(names(wt)[wt], quote = TRUE),
...
b_GET <- function(query,
  ...
  if (check)
    b_CHECK(res = res, contentType = contentType)
  else
    res
}
get_GET_async <- function(query,
                          api,
                          check = FALSE,
                          contentType = 'text/html; charset=utf-8',
                          ...) {
  urls <- b_url_async(api = api, query = query)
  cli <- crul::Async$new(urls = urls)
  res <- cli$get(...)
  if (check) {
    lapply(res, b_CHECK, contentType = contentType)
  } else {
    res
  }
}
...
b_read_tsv <- function(x,
                       header = TRUE,
                       sep = "\t",
                       cleanData = FALSE,
                       ...) {
  x <- data.table::setDF(data.table::fread(
    text = x,
    header = header,
    sep = sep,
    ...

...
if (!interactive() &&
  ...
  test_that("bold_tax_id2 returns the correct object (response)", {
    ...
    expect_equal(test$response$response_headers$`content-type`,
  ...
  test_that(
    "bold_tax_id2 'dataTypes' param works as expected (stats without public marker seqs)",
    {
      vcr::use_cassette("bold_tax_id2", {
  ...
  test_that(
    "bold_tax_id2 'dataTypes' param works as expected (basic & stats; as written for bold_tax_id)",
    {
      vcr::use_cassette("bold_tax_id2", {
  ...
  test_that("bold_tax_id2 fails well", {
    ...
    expect_error(bold_tax_id2(id = 88899, dataTypes = 5),
                 "'dataTypes' must be of class character")
    expect_error(bold_tax_id2(id = 88899, dataTypes = "base"),
                 "'base' is not a valid dataTypes")
    expect_error(bold_tax_id2(id = 88899, dataTypes = c("base", "pic")),
                 "'base' and 'pic' are not valid dataTypes")
    expect_error(
      bold_tax_id2(id = 88899, includeTree = 5),
      "'includeTree' should be one of TRUE or FALSE"
    )
  })
}
